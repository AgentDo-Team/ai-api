...
1. `aggregate_score`(2차 매칭도) 내림차순 **상위 10건**(`CANDIDATE_N`)으로 채점 후보를 좁히고
2. 후보 공고를 평가기준표(배점표) 기반으로 채점하고 (`EvaluationService`)
3. 최종점수(`final_score`, 배점표 채점 점수) 기준 **상위 5개 공고에만** 적합/부적합 LLM 검증과 100자 요약을 수행한 뒤
4. 그 결과를 응답으로 반환하면서 `analysis_results` 테이블에도 저장한다.

`aggregate_score`(2차 필터가 산출한 매칭도)는 **1단계 후보 선별에만** 쓰고, 최종점수 산정에는 **쓰지 않는다**. 2차 필터가 이미 반영한 값이라 점수로 다시 더하면 이중 계산이 되지만, 어느 공고를 채점할지 고르는 사전 지표로는 유효하다. 응답에는 참고용으로 그대로 노출된다. `final_score` 는 DB 상으로는 `analysis_results.soft_score` 컬럼에 저장되지만, 응답 DTO 에는 `soft_score` 라는 별도 필드로 중복 노출하지 않고 `final_score` 하나로만 내려준다.

후보 컷은 성능 때문이다. 채점은 공고당 LLM 호출이 수십 회로 파이프라인에서 가장 비싼 단계인데 최종 반환은 5건뿐이라, 컷이 없으면 하드필터를 통과한 공고 수에 비례해 응답 시간이 늘어난다. 컷 대상 공고는 오류가 아니므로 `skipped` 에 들어가지 않고 조용히 제외되며, `analysis_results` 행도 생성되지 않는다(= `GET /bid-notices/search-sets/{id}/analysis-results` 는 최대 10건을 돌려준다).

**동기 API**다. 후보 10건 상한 + 공고 간 병렬(동시 3건) + 공고 내 세부항목 병렬(동시 5개) 채점으로, 공고 1건당 약 10초 안팎 — 후보 10건 기준 대략 **40~60초**가 걸린다. 입력 공고가 이보다 많아도 채점 대상은 10건으로 고정이라 응답 시간의 상한이 생긴다. 프론트/프록시의 HTTP 타임아웃은 여유 있게 **2~3분**으로 잡는 것을 권장한다(세부항목이 많은 RFP, LLM rate limit 등 변수가 있다).
...
| `results[].aggregate_score` | 공고 전체 매칭도 (2차 필터의 청크 점수 집계). **채점 후보 상위 10건을 고르는 기준**으로 쓰인다. 응답에 참고용으로 그대로 노출되며 최종점수 산정에는 쓰이지 않는다 |
...
`final_score` 는 배점표 채점 점수 그 자체다. `aggregate_score` 는 채점 후보를 고르는 데 쓰이고, 응답에는 참고 정보로 포함되지만 최종점수/정렬에는 반영되지 않는다.
...
POST /api/third-filter
        │
        ▼
[1단계] aggregate_score 내림차순 상위 10건(CANDIDATE_N)으로 채점 후보 선별
        │   (채점이 가장 비싼 단계인데 최종 반환은 5건뿐이라 미리 좁힌다.
        │    컷된 공고는 오류가 아니므로 skipped 에 들어가지 않고 조용히 제외)
        ▼
[2단계] 후보 공고 배점표 채점 ── 공고 간 병렬 (Semaphore=3, 공고별 독립 DB 세션)
        ...
        ▼
[3단계] final_score(= soft_score) 내림차순 정렬 → 상위 5개 선별
        │   (aggregate_score 는 후보 선별에만 쓰고 최종점수 산정에는 쓰지 않는다.
        │    2차 필터가 이미 반영한 값이라 점수로 다시 더하면 이중 계산이 되므로,
        │    후보 안에서의 순위는 배점표 채점 결과만으로 매긴다)
        ▼
[4단계] 상위 5개만 검증 + 요약 ── 공고 간 병렬 (공고별 독립 DB 세션)
        ...
        ▼
[5단계] ThirdFilterResponse 조립 → ApiResponse 로 감싸 반환
```
...
채점 대상은 `CANDIDATE_N = 10` 이 상한이므로, 입력 공고가 아무리 많아도 응답 시간은 아래 10건 행에서 더 늘지 않는다.

| 입력 공고 수 | 채점 대상 | 총 LLM 호출(항목 5개 가정) | 예상 응답 시간 |
|---|---|---|---|
| 5건 | 5건 | ~40회 | ~25초 |
| 10건 | 10건 | ~70회 | ~45–60초 |
| 20건 이상 | 10건 (컷) | ~70회 | ~45–60초 |

> 위 표의 "항목 5개"는 낙관적 가정이다. 실제 `analysis_results` 기준 공고당 세부평가 항목은 **평균 18개**(최대 61개)라, 실제 호출 수와 소요 시간은 표보다 크다. 항목 수가 곧 공고당 LLM 호출 수이므로, 배점표가 긴 RFP 일수록 느려진다.
