...
회사/협업사/보완이 필요한 입찰공고 정보를 조회하고, 부족한 부분에 대한 설명을 LLM으로
생성해 메일 양식(제목/본문)에 채워 넣는다.

LangGraph 파이프라인에서는 메일 작성(compose) → HITL 검수 → 발송
(app.clients.gmail_client.send_email) 순서로 쓰일 예정이라, 이 서비스는 DB 저장이나
발송 없이 초안만 반환한다.
"""
...
class CollaborationEmailService:
    ...
    async def compose(
        ...
        partner = await self.company_repo.get(partner_company_id)
        if partner is None:
            raise AppException("협업할 회사를 찾을 수 없습니다.", status_code=404)
        ...
        return CollaborationEmailDraft(subject=subject, body=body)
