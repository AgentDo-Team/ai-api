...
# 💡 비동기 함수로 변경
async def process_and_save_bids(results, out_dir="pipeline_out"):
    """파일을 다운로드/파싱한 후, Repository를 통해 DB에 적재합니다."""
    ...
    # 💡 동기 Session 대신 비동기 AsyncSession 사용
    async with AsyncSession(engine, expire_on_commit=False) as session:
        ...
        for r in results:
            print(f"\n⚙️ [{r['notice_no']}] 처리 중: {r['title']}")
            
            # 💡 1. Repo 함수를 이용한 중복 조회
            existing = await bid_repo.get_by_notice_no(r['notice_no'])
            if existing:
                print(f"  [건너뜀] 이미 DB에 존재하는 공고입니다.")
                continue

            # 파일 다운로드 (동기 requests 유지)
            ...
            # 💡 2. Repo 함수를 이용한 추가 (add)
            try:
                await bid_repo.add(new_notice)
                
                # DB 커밋은 여기서 진행 (Service 없이 스크립트로 짤 경우)
...
async def run_bid_pipeline():
    ...
    bid_results = collect_bids(days_back=7)
    if bid_results:
        await process_and_save_bids(bid_results)
    else:
